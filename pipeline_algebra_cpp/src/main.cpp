...
Matrix compute_kernel_block_batched(
  ...
  for (int start_i = 0; start_i < n_nodes; start_i += batch_size) {
    ...
    for (int start_j = start_i; start_j < n_nodes; start_j += batch_size) {
      ...
      std::cout << "Batch: row= " << start_i << ", column= " << start_j
                << std::endl;
...
std::pair<Matrix, std::vector<int>> compute_kernel_matrix_blockwise(
  ...
  for (const auto &block : blocks) {
    ...
    //
    Matrix K_block = compute_kernel_block_batched_gpu(
        c, i, s, firstHopStates, node_ids, data, nodeDegrees, nodeDegreesC1,
        nPower);
    ...
    // ---- Save checkpoint ----
    std::ostringstream fname_block;
    fname_block << "output/K_block_c" << c << "_i" << i << ".npy";
    save_matrix_npy(K_block, fname_block.str());

    std::ostringstream fname_total;
    fname_total << "output/K_total_after_block_c_" << c << "_i_" << i << ".npy";
    save_matrix_npy(K_total, fname_total.str());

    std::cout << "[Block c:" << c << " i: " << i
              << "] Saved intermediate results: " << fname_block.str()
              << " and " << fname_total.str() << std::endl;
  }
