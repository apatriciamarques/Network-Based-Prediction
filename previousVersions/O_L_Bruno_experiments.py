...
qc = QuantumCircuit(qr_z,qr_k,qr_v_temp,qr_l,qr_a1,qr_a2,qr_v)#reondernei a ordem do qubits
...
def oracle_O_L(qc, qr_l, qr_v, qr_v_temp, Adjacency_list):
    for j in range(len(Adjacency_list)):
        for i, integer in enumerate(Adjacency_list[j]):
            ...
            print(ctrl_state1=i,)
            controlled_gate = create_double_controlled_gate(base_gate=my_gate,num_ctrl_qubits1=m,ctrl_state1=i,num_ctrl_qubits2=n,ctrl_state2=j);
            qc.append(controlled_gate, [*, *qr_v, *qr_v_temp])
