...
def swap_registers(qc, reg1, reg2):
    """
    Swap all qubits between two registers of equal size
    
    Args:
        qc (QuantumCircuit): The quantum circuit
        reg1 (QuantumRegister): First quantum register
        reg2 (QuantumRegister): Second quantum register to swap with
    """
    if len(reg1) != len(reg2):
        raise ValueError("Registers must be of equal size")
        
    # Swap corresponding qubits between registers
    for i in range(len(reg1)):
        qc.swap(reg1[i], reg2[i])
...
def create_double_controlled_gate(base_gate: Gate, num_ctrl_qubits1: int, ctrl_state1: int, 
                                num_ctrl_qubits2: int, ctrl_state2: int):
    """
    Creates a gate controlled by two separate registers with different control states
    
    Args:
        base_gate: The gate to be controlled
        num_ctrl_qubits1: Number of qubits in first control register
        ctrl_state1: Control state for first register (binary number)
        num_ctrl_qubits2: Number of qubits in second control register
        ctrl_state2: Control state for second register (binary number)
    """
    # First create the control with the first register
    intermediate_gate = base_gate.control(num_ctrl_qubits=num_ctrl_qubits1, 
                                        ctrl_state=ctrl_state1)
    
    # Then add control from the second register
    final_gate = intermediate_gate.control(num_ctrl_qubits=num_ctrl_qubits2,
...
qr_l = QuantumRegister(m, 'l')  # Basis-encode non-zero entries of each row of A
qr_v = QuantumRegister(n, 'v')  # Basis-encode node index
qr_v_temp = QuantumRegister(n, 'v_temp')  # Basis-encode node index
qr_k = QuantumRegister(m, 'k')  # Basis-encode degree value (k)
qr_z = QuantumRegister(p, 'z')  # Basis-encode feature value (x)
qr_a1 = QuantumRegister(1, 'a1')  # Ancilla qubit for rotation (feature value)
qr_a2 = QuantumRegister(1, 'a2')  # Ancilla qubit for rotation (degree inverse)
...
Adjacency_list=[[1,12,15],[4,5,8],[6,8,10]] #31 e 


qc = QuantumCircuit(qr_z,qr_k,qr_v_temp,qr_v,qr_l,qr_a1,qr_a2)#reondernei a ordem do qubits
...
def oracle_O_L(qc, qr_l, qr_v, qr_v_temp, Adjacency_list):
    for j in range(len(Adjacency_list)):
        for i, integer in enumerate(Adjacency_list[j]):
            #print(j,i,integer)
            # First create the base gate
            my_gate = IntegerToRegisterGate(integer, n) 
            # First control layer - control on qr_l
            controlled_gate = my_gate.control(num_ctrl_qubits=n+m,ctrl_state=i+j*2**m);
            qc.append(controlled_gate, [*qr_l, *qr_v, *qr_v_temp])
...
oracle_O_L(qc,qr_l, qr_v,qr_v_temp, Adjacency_list)
swap_registers(qc, qr_v, qr_v_temp)
print("\n6. test:"), access_statevector(qc)
qc.h(qr_v_temp)
qc.h(qr_l)
qc.reset(qr_l)
